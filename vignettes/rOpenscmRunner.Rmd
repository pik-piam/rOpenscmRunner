...
to install `openscm-runner` with all dependencies for the models. If you use `pip`, run from the command line:

```bash
$ pip install --user openscm-runner[models]
```
...
it knows where to finde them. However, MAGICC is installed separately, so you have to tell `rOpenscmRunner` where to
find it using the `rOpenscmRunner::setup` function:

```{r}
rOpenscmRunner::setup(magiccExecutable7 = "/path/to/magicc/bin/magicc")
```
...
```{r}
rOpenscmRunner::setup(magiccWorkerNumber = 4, magiccWorkerRootDir = "/path/to/temp/folder")
```
...
clusters.
The default for the temporary folder is to use your system's default temporary folder location. You might want to
specify another temporary folder here, for example to take advantage of a fast in-memory file system. Note however
that you are responsible to create and clean up the temporary folder yourself if you do specify one.
...
a time series and the columns are `model` for the model that produced the emissions scenario, `scenario` for a moniker
...
documentation. The names of `outputVariables` is standardized for the most common output variables by
`openscm-runner` according to the names used in the [SR15 database](https://data.ene.iiasa.ac.at/iamc-1.5c-explorer)
...
It is also possible to run multiple models at once. Expanding on the sea level rise example, we could als add
